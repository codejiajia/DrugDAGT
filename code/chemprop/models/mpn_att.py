...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, (a_start, a_size) in enumerate(a_scope):  # a_scope: (1, num_atoms)
            ...
            if a_size == 0:
                ...
                if self.atom_attention:
                    ...
                    tom_vecs.append(mol_vec)
                    if viz_dir and (a_size > 1):
        ...
        tom_vecs = torch.cat(tom_vecs, dim=0)
        batch_indices = torch.tensor(batch_indices, dtype = int).to(self.device)

        return mol_vecs, tom_vecs, batch_indices  # num_molecules x hidden
...
class MPN(nn.Module):
    ...
    def forward(self,
        ...
        output, atom_vecs, batch_indices = encodings[0]
            
        return output, atom_vecs, batch_indices
