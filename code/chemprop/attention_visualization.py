...
def shorten_filename(filename, limit = 254):
    if len(filename) <= limit:
        return filename
    else:
        return filename[:50] + '...' + filename[(len(filename) - 50):]
...
def visualize_atom_attention(viz_dir: str,
    ...
    std_weight = np.std(atomSum_weights, axis = 0)
    nanMean = np.nanmean(Amean_weight)

    filename = shorten_filename(mol_name)
    save_path = os.path.join(
        smiles_viz_dir, f'{filename.replace("/", "")}.png')

    fig = SimilarityMaps.GetSimilarityMapFromWeights(mol, Amean_weight-nanMean,
                                                     alpha=0.5,
                                                     size = (300,300))

    # fig = SimilarityMaps.GetSimilarityMapFromWeights(mol, list(std_weight),
    #                                                  alpha=0.3,
    #                                                  size = (300,300))
