...
def run_training(args: TrainArgs,
    ...
    if args.separate_test_path:
        test_data = get_data(path=args.separate_test_path,
                             ...
                             logger=logger)
        test_data_right = get_data(path=args.separate_test_path_right,
                             args=args,
                             features_path=args.features_path,
                             adjacency_path=args.adjacency_path,
                             distance_path=args.distance_path,
    ...
    if args.separate_val_path:
        val_data = get_data(path=args.separate_val_path,
                            ...
                            smiles_columns=args.smiles_columns,
                            logger=logger)
        val_data_right = get_data(path=args.separate_val_path_right,
                            args=args,
                            features_path=args.features_path,
                            adjacency_path=args.adjacency_path,
                            distance_path=args.distance_path,
    ...
    train_data_right_loader = MoleculeDataLoader(
        dataset=train_data_right,
        batch_size=args.batch_size,
        num_workers=num_workers,
        class_balance=args.class_balance,
        shuffle=True,
    ...
    val_data_right_loader = MoleculeDataLoader(
        dataset=val_data_right,
        batch_size=args.batch_size,
        num_workers=num_workers
    )
    ...
    test_data_right_loader = MoleculeDataLoader(
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            n_iter = train(
                ...
                data_right_loader=train_data_right_loader,
                loss_func=loss_func,
                optimizer=optimizer,
                scheduler=scheduler,
                args=args,
            ...
            if isinstance(scheduler, ExponentialLR):
                scheduler.step()
            val_scores, left_val_feat, right_val_feat = evaluate(
                model=model,
                data_loader=val_data_loader,
                data_right_loader=val_data_right_loader,
            ...
            if epoch % 5 == 0:
                np.save(os.path.join(args.save_dir, 'left_val_feature_epoch' + str(epoch) + '.npy'), left_val_feat)
                np.save(os.path.join(args.save_dir, 'right_val_feature_epoch' + str(epoch) + '.npy'), right_val_feat)
        ...
        test_preds, left_att, right_att, left_feat, right_feat = predict(
            model=model,
            data_loader=test_data_loader,
            data_right_loader=test_data_right_loader,
            scaler=scaler
        )
        ...
        np.save(os.path.join(args.save_dir, 'left_attention.npy'), left_att)
        np.save(os.path.join(args.save_dir, 'rightt_attention.npy'), right_att)
        np.save(os.path.join(args.save_dir, 'left_feature.npy'), left_feat)
        np.save(os.path.join(args.save_dir, 'rightt_feature.npy'), right_feat)
        np.save(os.path.join(args.save_dir, 'predict_test.npy'),np.array(test_preds))
